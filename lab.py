...
test_key = PBKDF2(password, salt, dkLen=32) #AES256 requires a length of 32 bytes for the key, otherwise its just setting up a key for this message.
print(test_key)

message = b"I want to kill the pope"

cipher = AES.new(test_key, AES.MODE_CBC) #CBC is a mode of operation for block ciphers, it requires an initialization vector, which is generated by the cipher object. <-- thanks github
